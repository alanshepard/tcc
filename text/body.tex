...
The temperatures and pressure ratio can then be readily calculated from
\begin{align}
...
\end{equation}
This pressure ratio loss term must be multiplied to the ideal (lossless) pressure ratio given by the second term of \cref{eqn:turb_res_pr}. If more than one blade row is considered (e.g.\ one for the stator and another one for the rotor) the pressure ratio losses for each must be joined by multiplication.
