...
\begin{figure}
    ...
    \caption*{The performance predicted by the semi-empirical model data shows the trends exhibited by the experimental data, i.e. the \ac{EGT} is mostly flat and the \ac{F} and compressor pressure ratio ($\pr{03}{02}$) rise quadratically with the compressor tip Mach number ($M_{bc}$).
    The large (approximately 200\%) quantitative error is probably due to the overly optimistic efficiency of the model components. The model data is only available up to the engine inlet choke. Experimental data was obtained by \textcite{bolsoni_test}.}
\end{figure}
