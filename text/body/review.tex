...
\begin{equation}
    \Delta h = \frac{\text{power}}{\dot{m}} 
             = \vec{\omega} \cdot (\vec{r}_{\acs{e}}\cross\vec{V}_{\acs{e}} - \vec{r}_{\acs{i}}\cross\vec{V}_{\acs{i}}) 
\end{equation}

\begin{equation}
    \Delta h = \omega (r_{\acs{e}} {V_u}_{\acs{e}} - r_{\acs{i}} {V_u}_{\acs{i}}) 
\end{equation}

\begin{equation}
    \label{eqn:euler}
    \Delta h = U_{\acs{e}} {V_u}_{\acs{e}} - U_{\acs{i}} {V_u}_{\acs{i}} 
\end{equation}
...
$\psi=\frac{\Delta h}{U_5^2}$ is the power coefficient for the turbine.
...
\begin{equation}
    \delta = \frac{\frac{\rho_5 V_{x4}}{\rho_5 V_{x5}} s^{-1.1} (\alpha'_4+\beta_5)^{2.25}}
...
\begin{equation}
    B = \frac{1}{2a}\sqrt{\frac{\acs{volume}}{A L}}
\end{equation}
where $A$ and $L$ are the area and length of the pipe downstream from the compressor.
